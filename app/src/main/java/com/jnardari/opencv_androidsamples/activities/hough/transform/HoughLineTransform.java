 ...
 * <a href="http://homepages.inf.ed.ac.uk/rbf/HIPR2/hough.htm">
 *   http://homepages.inf.ed.ac.uk/rbf/HIPR2/hough.htm</a>
 * </p>
 * <p/>
 ...
 * all the possible values of theta.
 ...
 * which are usually found by some kind of edge detection.
 * Each of these coordinates is transformed into an r, theta curve.
 * This curve is discretised so we actually only look at a certain discrete number of theta values.
 ...
 * Each point in the array represents an (r, theta) value
 * which can be used to represent a line using the formula above.
 * </p>
 * <p/>
 * Once all the points have been added should be full of curves.
...
public class HoughLineTransform {
     ...
     * Called by the constructor so you don't need to call it yourself,
     * however you can use it to reset the transform if you want to plug in another image
     * (although that image must have the same width and height)
     */
    ...
    /**
     * Adds points from an image. The image is assumed to be greyscale black and white,
     * so all pixels that are not black are counted as edges. The image should have
     * the same dimensions as the one passed to the constructor.
     */
    ...
    /**
     * Adds points from an image (in OpenCV's org.opencv.core.Mat format).
     * The image is assumed to be greyscale black and white,
     * so all pixels that are not black are counted as edges.
     * The image should have the same dimensions as the one passed to the constructor,
     ...
     * You can use this method directly if your data isn't represented as a buffered image.
     */
    ...
    /**
     * Once points have been added in some way this method extracts the lines and returns them
     * as a Vector of HoughLine objects, which can be used to draw on the
     *
     * //@param percentageThreshold The percentage threshold above which lines are determined from
     * the hough array
     */
